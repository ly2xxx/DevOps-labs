...
### Quick Start
```powershell
# Setup Vault with sample secrets
...
$POD = oc get pod -n vault-demo -l app=vault-demo -o jsonpath='{.items[0].metadata.name}'
oc exec -n vault-demo $POD -- cat /vault/secrets/database
```
...
# Vault - Create secret
docker exec vault-dev vault kv put secret/myapp/db username=user password=pass

# OKD - Verify injection
oc logs -n vault-demo <pod-name> -c vault-agent
```

### Complete Guides
- **Full Lab Guide**: [VAULT-SECRETS-INTEGRATION.md](VAULT-SECRETS-INTEGRATION.md)
- **Command Reference**: [VAULT-QUICK-REFERENCE.md](VAULT-QUICK-REFERENCE.md)
