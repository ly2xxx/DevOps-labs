...
- [🔐 Advanced Lab: HashiCorp Vault Integration](#-advanced-lab-hashicorp-vault-integration)
...
## 🔐 Advanced Lab: HashiCorp Vault Integration

Once you're comfortable with the basics, try integrating **HashiCorp Vault** for external secrets management!

### What You'll Learn

- 🔒 Running HashiCorp Vault in Docker alongside OKD
- 🔑 Configuring Kubernetes authentication with Vault
- 📦 Storing application secrets centrally in Vault
- 🚀 Two methods to inject secrets into pods:
  - **Vault Agent Sidecar** - Dynamic secret injection at runtime
  - **External Secrets Operator** - Synced Kubernetes Secrets
- 🔄 Secret rotation and dynamic credentials
- 🎯 Production best practices for secret management

### Why Vault + OpenShift?

Instead of storing secrets directly in OpenShift:
...
- ✅ Industry-standard secret management workflow

### Get Started

📖 **Full tutorial:** [VAULT-SECRETS-INTEGRATION.md](VAULT-SECRETS-INTEGRATION.md)

**Quick Preview:**
```powershell
# Start Vault in Docker
docker run -d --name vault-dev -p 8200:8200 hashicorp/vault

# Store secrets in Vault
vault kv put secret/myapp/database username=dbuser password=secret123

# Deploy app with Vault-injected secrets
oc apply -f vault-demo-app.yaml

# Secrets automatically appear in pod at /vault/secrets/
```

**Time Required:** 1-2 hours for full lab  
**Prerequisites:** Completed basic OKD setup from this guide
