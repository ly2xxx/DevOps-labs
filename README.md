...
| ☸️ **Kubernetes** | [Kubernetes Hands-On Cheat Sheet](kubernetes/cheatsheet.md) | Operations guide for Docker Desktop Kubernetes workloads, ConfigMaps, Secrets, Ingress, Namespaces, and Helm. | `kubectl apply/describe/logs/scale/rollout`, ConfigMap & Secret injection, NodePort & Ingress routing, Namespaces, Helm charts. |
| ⎈ **Helm** | [Helm Hands-On Cheat Sheet](helm/cheatsheet.md) | Package management guide for Helm charts, multi-environment releases (Dev/Prod), and release lifecycles. | `helm template`, `helm install`, `helm upgrade`, `helm rollback`, `helm history`, multi-environment values (`values-dev.yaml`, `values-prod.yaml`). |
| 🔐 **HashiCorp Vault** | [HashiCorp Vault Cheat Sheet](token-rotation/cheatsheet.md) | Reference for server setup, secret engines administration, policies, and token rotation. | `vault server -dev`, unsealing keys, `vault kv put/get/delete`, access control policies, plugin catalog registration. |
| 📥 **Apache Kafka** | [Apache Kafka Cheat Sheet](kafka/cheatsheet.md) | CLI commands for managing Kafka brokers, topics, console producers/consumers, and consumer groups. | `kafka-topics.sh`, `kafka-console-producer.sh`, `kafka-console-consumer.sh`, `kafka-consumer-groups.sh`, cluster rebalancing & lag inspection. |
| 🤖 **OPA (Open Policy Agent)** | [Open Policy Agent Cheat Sheet](opa/cheatsheet.md) | Reference for policy authoring in Rego, evaluation query commands, and testing suites. | `opa check/fmt/eval/test`, Rego variables, loops (`some`/`every`), sets, K8s admission control webhook decision objects. |
| ⚙️ **Coder CLI** | [Coder CLI Cheat Sheet](coder-templates/cheatsheet.md) | Reference for developer workspaces provisioning, SSH access, template versioning, and background tasks. | `coder login/whoami`, `coder create/start/stop/delete`, `coder ssh/logs`, template pushing, background tasks & presets creation. |
| 🚀 **Docker DX** | [Docker DX Quick Start](docker-dx-extension/QUICKSTART.md) | Walkthrough for AI-powered Docker development inside VS Code using the new Docker DX extension. | Extension installation (`code --install-extension`), AI Dockerfile generation, container insights & performance panel, layer size optimization. |
